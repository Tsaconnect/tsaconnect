...
// Add these methods inside your APIService class
class APIService {
    // Handle API response
    ...
    // components/services/api.ts - Add this method to your APIService class

    // Add this method inside your APIService class (before the cart methods)
     ...
     */
    async cleanupExpiredCarts(): Promise<ApiResponse<{
        deletedCount: number;
        timestamp: string;
    }>> {
        try {
    ...
    async getCartItemCount(): Promise<number> {
        try {
            const response = await this.getOrCreateCart();
            if (response.success && response.data) {
                return response.data.items.reduce((count, item) => count + item.quantity, 0);
            }
            return 0;
        } catch (error) {
